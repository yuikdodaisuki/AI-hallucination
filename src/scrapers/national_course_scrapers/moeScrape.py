...
def scrape_all_pages(base_url, search_term, download_keywords, max_pages=1):
    """
    爬取教育部网站搜索结果的多个页面
...
def save_policy_data(policy_info, base_url, base_dir="../../data/teaching_achievements/national_courses"):
    """
    增强版数据存储函数
...
def startScrape():
    # 在这里对附件进行检索
    download = ["一流本科课程"] #检索词条
    scrape_all_pages("https://so.moe.gov.cn/", "国家级一流本科", download) # 修改：传入 download 列表
