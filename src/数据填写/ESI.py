...
def load_school_data(json_file_path):
    ...
    for school in data['results']:
        ...
        print(school)  # 输出学校名称以便调试
    #   "前1%数": 22,
    #   "前1‰数": 13,
    #   "全球排名": 17
        indicators_data['前1%数'][school_name] = school['前1%数']
        indicators_data['前1‰数'][school_name] = school['前1‰数']
        indicators_data['全球排名'][school_name] = school['全球排名']
...
def fill_standard_answers(csv_file_path, school_data_for_indicator, indicator_name, output_file_path=None):
    ...
    for index, row in df.iterrows():
        ...
        if school_name in school_data_for_indicator:
            ...
            df.at[index, '标准答案'] = '未找到数据' # 如果未找到学校，则填充为None
...
def process_single_indicator(base_dir, json_file, indicator_name, indicators_data):
    ...
    # 构建CSV文件路径
    csv_file_mapping = {
        '前1%数': 'ai_evaluation_dataset_long_ESI前1%学科数量_answers.csv',
        '前1‰数': 'ai_evaluation_dataset_long_ESI前1‰学科数量_answers.csv',
    }
...
def main():
    ...
    json_file = os.path.join(base_dir, "src/data/consolidated/ESI前1‰学科数量_data.json")
    ...
    try:
        ...
        total_schools = len(indicators_data['前1%数'])
